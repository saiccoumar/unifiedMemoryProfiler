...
static void advisePreferGpu(void* p, size_t n, int dev) {
    CHECK_CUDA(cudaMemAdvise(p, n, cudaMemAdviseSetPreferredLocation, dev));
}

static void prefetchToGpu(void* p, size_t n, int dev) {
    CHECK_CUDA(cudaMemPrefetchAsync(p, n, dev, 0));
}

static void prefetchToCpu(void* p, size_t n) {
    CHECK_CUDA(cudaMemPrefetchAsync(p, n, cudaCpuDeviceId, 0));
}
...
static ProfileResult runConcurrentAccess(size_t N, size_t bytes, int dev,
    ...
    r.alloc_ms = timeCpu([&]{
        ...
        CHECK_CUDA(cudaMemAdvise(u, bytes, cudaMemAdviseSetAccessedBy, dev));
        CHECK_CUDA(cudaMemAdvise(u, bytes, cudaMemAdviseSetAccessedBy, cudaCpuDeviceId));
    });
    memset(u, 0, bytes);
    cudaDeviceSynchronize();
